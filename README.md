[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/_iCgzE0B)
# Vocabulary Explorer
This is a medium-high fidelity prototype for an app called Vocabulary Explorer, which aims to help users discover how words in the Plains Cree language relate to each other.

## Installation
To install and run this app, download the .apk file from the Releases tab of this page, and run it on an Android device.

## Team Members
- Abhita Gupta (abhita)
- Alex Kumpula (ajkumpul)
- Jason Liang (zlinag12)
- Joshua Wong (jswong3)
- Zara Ezeasor (ezeasor)

## Resources Consulted
- [itwewina](https://itwewina.altlab.app) was consulted to gather sample data for the project.
- Gemini was used to help in the codebase
